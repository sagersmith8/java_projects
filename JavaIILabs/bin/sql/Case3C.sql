...
select ITEM.ITEM_DESC from ITEM,INVENTORY, ORDER_LINE WHERE INVENTORY.INV_ID = ORDER_LINE.INV_ID AND ORDER_LINE.O_ID = 1 AND ITEM.ITEM_ID = INVENTORY.ITEM_ID;
...
SELECT CUSTOMER.C_FIRST||' ' ||CUSTOMER.C_LAST AS NAME, SUM(INVENTORY.INV_PRICE*ORDER_LINE.OL_QUANTITY) AS PRICE FROM CUSTOMER, INVENTORY, ORDERS, ORDER_LINE WHERE CUSTOMER.C_ID = ORDERS.C_ID AND ORDERS.O_ID = ORDER_LINE.O_ID AND INVENTORY.INV_ID = ORDER_LINE.INV_ID AND ORDER_LINE.O_ID = 1 GROUP BY CUSTOMER.C_LAST, CUSTOMER.C_FIRST;
--NAME                                                               PRICE
------------------------------------------------------------- ----------
--Neal Graham 														379.89

SELECT CUSTOMER.C_FIRST||' ' ||CUSTOMER.C_LAST AS NAME, SUM(ORDER_LINE.OL_QUANTITY) AS QUANTITY FROM CUSTOMER, ITEM, INVENTORY, ORDERS, ORDER_LINE WHERE CUSTOMER.C_ID = ORDERS.C_ID AND ORDERS.O_ID = ORDER_LINE.O_ID AND INVENTORY.INV_ID = ORDER_LINE.INV_ID AND ITEM.ITEM_ID = INVENTORY.ITEM_ID AND ITEM.ITEM_DESC LIKE 'Boy''s Surf Shorts' GROUP BY CUSTOMER.C_LAST, CUSTOMER.C_FIRST;
--no rows selected

SELECT DISTINCT INVENTORY.ITEM_ID, SHIPMENT_LINE.SHIP_ID FROM SHIPMENT_LINE, INVENTORY WHERE SHIPMENT_LINE.INV_ID = INVENTORY.INV_ID AND SHIPMENT_LINE.SL_DATE_RECEIVED IS NULL;
--   ITEM_ID    SHIP_ID
---------- ----------
  --       2          2
    --     3          3

CREATE VIEW ITEMS AS SELECT INV_ID, INV_PRICE, INV_QOH FROM INVENTORY;
...
---------- ---------- ----------
        ...
        --31      19.99        100
       -- 32      19.99        100
SELECT SUM(INV_PRICE*INV_QOH) FROM ITEMS;
--SUM(INV_PRICE*INV_QOH)
----------------------
            -- 126224.29
SELECT SHIPMENT.SHIP_ID, SHIPMENT.SHIP_DATE_EXPECTED, SHIPMENT_LINE.SL_DATE_RECEIVED FROM SHIPMENT, SHIPMENT_LINE WHERE SHIPMENT.SHIP_ID = SHIPMENT_LINE.SHIP_ID;      

   --SHIP_ID SHIP_DATE SL_DATE_R
---------- --------- ---------
         --1 15-SEP-06 10-SEP-06
         --1 15-SEP-06 10-SEP-06
         --2 15-NOV-06
         --3 25-JUN-06
         --3 25-JUN-06
         --3 25-JUN-06
        -- 4 25-JUN-06 15-AUG-06
      --   4 25-JUN-06 25-AUG-06
    --     5 15-AUG-06 15-AUG-06
  --       5 15-AUG-06 15-AUG-06
--         5 15-AUG-06 15-AUG-06
...
FROM ORDER_LINE, INVENTORY, ITEM, CATEGORY 
WHERE ORDER_LINE.INV_ID = INVENTORY.INV_ID 
AND CATEGORY.CAT_DESC LIKE 'Outdoor Gear' 
AND CATEGORY.CAT_ID = ITEM.CAT_ID 
AND ITEM.ITEM_ID = INVENTORY.ITEM_ID; 

--SUM(OL_QUANTITY*INV_PRICE)
--------------------------
  --                  519.98

SELECT SUM(INVENTORY.INV_PRICE * ORDER_LINE.OL_QUANTITY) 
FROM INVENTORY, ORDER_SOURCE, ORDERS, ORDER_LINE
WHERE ORDER_SOURCE.OS_ID = ORDERS.OS_ID
AND ORDERS.O_ID = ORDER_LINE.O_ID
AND INVENTORY.INV_ID = ORDER_LINE.INV_ID
AND ORDER_SOURCE.OS_DESC LIKE 'Web Site';


--SUM(INVENTORY.INV_PRICE*ORDER_LINE.OL_QUANTITY)
-----------------------------------------------
 --                                        105.89
