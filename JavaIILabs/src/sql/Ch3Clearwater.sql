...
---------------------------------------------------------------
--------------------------------------------------------------
SELECT INV_PRICE FROM INVENTORY WHERE INV_ID LIKE 1;
 INV_PRICE
----------
    259.99
    
SELECT C_FIRST, C_MI, C_LAST FROM CUSTOMER WHERE C_BIRTHDATE BETWEEN '01/JAN/1970' AND '31/DEC/1979';
C_FIRST                        C C_LAST
------------------------------ - -------
Sheila                         A Lewis
Thomas                         E James

SELECT DISTINCT COUNT(CAT_DESC) FROM CATEGORY;

COUNT(CAT_DESC)
---------------
              4
              
SELECT COUNT(DISTINCT SHIP_ID) FROM SHIPMENT_LINE WHERE SL_DATE_RECEIVED IS NULL;

COUNT(DISTINCTSHIP_ID)
----------------------
                     2
...
---------- ------------
 ...
 SELECT COUNT(O_ID) FROM ORDERS WHERE O_DATE = '29/MAY/2006';

 COUNT(O_ID)
-----------
          2
          
 SELECT COUNT(O_METHPMT) FROM ORDERS WHERE O_DATE = '31/MAY/2006' AND O_METHPMT = 'CC';
 
 COUNT(O_METHPMT)
----------------
               1
 
 SELECT C_LAST, C_FIRST FROM CUSTOMER WHERE C_STATE IN('FL','GA');
...
SELECT * FROM INVENTORY WHERE INV_SIZE IS NULL;
...
SELECT * FROM SHIPMENT WHERE SHIP_DATE_EXPECTED = '1/SEP/2006';
no rows selected

SELECT * FROM INVENTORY WHERE INV_PRICE > 60 AND INV_SIZE IN('XL','L');
  INV_ID    ITEM_ID COLOR                INV_SIZE    INV_PRICE    INV_QOH
-------- ---------- -------------------- ---------- ---------- ----------
      25          1 Spruce               L              209.95          0
      26          1 Spruce               XL             209.95         12
      
SELECT COUNT(DISTINCT COLOR) FROM INVENTORY WHERE ITEM_ID = 5;


COUNT(DISTINCTCOLOR)
--------------------
                   2
SELECT COUNT(DISTINCT INV_SIZE) FROM INVENTORY WHERE ITEM_ID=5;

COUNT(DISTINCTINV_SIZE)
-----------------------
                      4
SELECT C_FIRST, C_LAST, MONTHS_BETWEEN(SYSDATE,C_BIRTHDATE)/12 AS AGE FROM CUSTOMER;
C_FIRST                        C_LAST                                AGE
------------------------------ ------------------------------ ----------
Neal                           Graham                         46.2595074
Myra                           Sanchez                        55.5820881
Lisa                           Smith                          53.9207977
Paul                           Phelp                          33.1546687
Sheila                         Lewis                          35.5390773
Thomas                         James                           40.783701

SELECT TO_CHAR(SHIP_DATE_EXPECTED,'MONTH/DD/YYYY') FROM SHIPMENT;
TO_CHAR(SHIP_DATE
-----------------
SEPTEMBER/15/2006
NOVEMBER /15/2006
...
SELECT OS_ID, COUNT(OS_ID) FROM ORDERS GROUP BY OS_ID;
     OS_ID COUNT(OS_ID)
---------- ------------
         6            2
         2            2
 ...
 SELECT ITEM_ID FROM INVENTORY WHERE INV_QOH>0 AND INV_SIZE IN('M','L') AND COLOR IN('Royal', 'Bright Pink','Spruce') GROUP BY ITEM_ID;
   ITEM_ID
----------
         1
         4
  
 SELECT DISTINCT ITEM_ID, COLOR FROM INVENTORY GROUP BY ITEM_ID, COLOR ORDER BY ITEM_ID;
   ITEM_ID COLOR
---------- -------------
         1 Spruce
         2 Light Grey
 ...
 SELECT COUNT(ITEM_ID) FROM ITEM WHERE CAT_ID NOT IN(2,4);
 COUNT(ITEM_ID)
--------------
             3

SELECT DISTINCT ITEM_ID, TO_CHAR(INV_PRICE,'$999.99') FROM INVENTORY ORDER BY ITEM_ID;
   ITEM_ID TO_CHAR(
---------- --------
         1  $199.95
         1  $209.95
